...
class ChatManager {
  ...
  // Close a ticket
  closeTicket(ticketId: string, closedBy: string): any {
    const tickets = this.getAllTickets()
  ...
  getMessages(ticketId: string): any[] {
    if (typeof window === 'undefined') return []
  ...
  getDefaultMessages(ticketId: string): any[] {
  ...
  // Save messages
  saveMessages(ticketId: string, messages: any[]): void {
    if (typeof window !== 'undefined') {
  ...
  // Add new message
  addMessage(ticketId: string, message: any): any {
