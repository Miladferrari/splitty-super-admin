...
class SendBirdManager {
  ...
  async connect(userId: string, nickname: string = ''): Promise<any> {
    // Rate limit check
    const now = Date.now()
  ...
  async _performConnection(userId: string, nickname: string): Promise<any> {
  ...
  _connectUser(sb: any, userId: string, nickname: string, resolve: Function, reject: Function): void {
  ...
  async rateLimitedCall(key: string, callback: Function): Promise<any> {
    const now = Date.now()
