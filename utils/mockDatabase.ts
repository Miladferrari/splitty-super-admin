...
class MockDatabase {
  ...
  getOrder(orderId: string | number): any {
  ...
  getPayment(paymentId: string | number): any {
    // Try both string and number versions
  ...
  getOrdersByRestaurant(restaurantId: string | number): any[] {
    return Array.from(this.orders.values()).filter(order => 
      order.restaurantId === parseInt(restaurantId)
    );
  }

  getPaymentsByRestaurant(restaurantId: string | number): any[] {
  ...
  getLiveOrdersForRestaurant(restaurantId: string | number): any[] {
